...
static void _browser_read_var(void)
{
    ...
    {
        temp_val.g_comval.magic = VRAM_MAGIC(VM_AP_SETTING);
        com_get_default_setting(&temp_val);
        sys_vm_write(&temp_val, VM_AP_SETTING, sizeof(temp_val));
    }
    ...
    // 第一次进入AP,初始化VM变量
    if (g_browser_var.magic != VRAM_MAGIC(VM_AP_BROWSER))
    {
        g_browser_var.magic = VRAM_MAGIC(VM_AP_BROWSER);
...
*/
        ...
        need_send_msg = FALSE;
        //applib_start_playlist_builder(APP_ID_PLAYLIST, APP_ID_BROWSER|PLIST_DISK_H);
        msg.content.data[0] = APP_ID_PLAYLIST;
        #ifdef _CARD_BASE_
        msg.content.data[1] = APP_ID_BROWSER|PLIST_DISK_C;
        #else
        msg.content.data[1] = APP_ID_BROWSER|PLIST_DISK_H;
        #endif
        break;
        default:
        //默认的其它返回值均返回main
...
int main(int argc, char *argv[])
{
    ...
    //保存AP VM变量
    sys_vm_write(&g_browser_var, VM_AP_BROWSER, sizeof(browser_var_t));
