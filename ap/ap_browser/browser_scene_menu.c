...
app_result_e browser_menu_option(file_path_info_t *path_info, uint16 active)
{
    ...
    dir_brow_t brow;//获取当前目录层级
    brow.name_buf = NULL;
    brow.name_len = 0;
    fsel_browser_enter_dir(CUR_DIR, 0, &brow);
    flag5 = brow.layer_no;
  
    uint8 dir_name_buffer[32];//获取当前目录名字
    play_sta = get_engine_state();
    flag1 = vfs_cd(browser_mount, CD_BACK, NULL);
    if(0 == flag1)//cd fail
    {
    	flag2 = -1;//不在RECORD目录下
    }
    ...
    {
        flag2 =  vfs_get_name(browser_mount, dir_name_buffer, 31);
        flag3= libc_strncmp(dir_name_buffer, rec_dir_name, sizeof(rec_dir_name));
	flag4 = libc_strncmp(dir_name_buffer, record_name,sizeof(record_name));
	vfs_cd(browser_mount, CD_SUB, NULL);
    }
   //当前无播放且不在RECORD目录下面,不显示播放
   if ((play_sta == ENGINE_STATE_NULL)&& 
   	((1 != flag5) ||( (0 != flag3)&&(0 != flag4))))
    {
        gui_dialog_msg(DIALOG_MSG, DIALOG_BUTTON_ANYKEY, S_OPTION_DESC);
        return RESULT_REDRAW;
    }
 //当前无播放且在RECORD目录下,显示删除
    else if((play_sta == ENGINE_STATE_NULL)&& 
	((0 == flag4) ||(0 == flag3))&&(1 == flag5))
    {
        op_menu.menu_entry = BROWSER_MENU_ENTRY_NOPLAY;
    }
//当前正在播放且不在RECORD目录下，只显示正在播放
    else if ((play_sta == ENGINE_STATE_PLAYING)&&
	((1 != flag5) ||( (0 != flag3)&&(0 != flag4))))
    {
        op_menu.menu_entry = BROWSER_MENU_ENTRY_PLAYING_ONLY;
    }
//当前正在播放且在RECORD目录下,显示删除+正在播放
    else if ((play_sta == ENGINE_STATE_PLAYING)&& 
	((1 != flag5) ||( (0 != flag3)&&(0 != flag4))))
    {
        op_menu.menu_entry = BROWSER_MENU_ENTRY_PLAYING;
    }
//当前暂停状态且由不在RECORD目录下,只显示上一次播放
    else if ((play_sta == ENGINE_STATE_PAUSE)&&
	((1 != flag5) ||( (0 != flag3)&&(0 != flag4))))
    {
        op_menu.menu_entry = BROWSER_MENU_ENTRY_LASTPLAY_ONLY;
    }
////当前暂停状态且在RECORD目录下, 显示上一次播放+删除
    else if ((play_sta == ENGINE_STATE_PAUSE)&&
	((1 != flag5) ||( (0 != flag3)&&(0 != flag4))))
    {
