...
int standby(void)
{
    ...
    //bak_clk = act_readl(CLKENCTL); //gl5110 hjh 2015-10-17
    bak_clk = act_readl(CMU_DEVCLKEN);//gl5116 hjh 2015-10-17
    bak_mrcr = act_readl(MRCR);

    //考虑功耗的话，还需要关闭一些模块
    //act_writel(0x00000c00, CLKENCTL); //disable all module clk, except USB //gl5110 hjh 2015-10-17
    act_writel(0x00000c00, CMU_DEVCLKEN); //disable all module clk, except USB //gl5116 hjh 2015-10-17
    ...
    while (1)
    {
        ...
        if ((act_readb(CHG_DET) & 0x80) == 0) //gl5116 hjh 2015-10-13
        {
            //switch_off();//此处导致死机
            {
                ...
                ONOFF_KEY); //gl5116 hjh 2015-10-13
                //断电
                /* wait for power off because of capacitance effect.  xiaomaky.  2013/1/25 11:38:32.  */
                act_writel(act_readl(POWER_CTL) & (~(0x00000001 << POWER_CTL_EN_S1)), POWER_CTL);
        ...
        //play键，退出循环
        //if ((act_readl(SYSTEM_CTL_RTCVDD) & 0x80000000) != 0) //gl5110 hjh 2015-10-13
        if ((act_readl(ONOFF_KEY) & 0x1) != 0) //gl5116 hjh 2015-10-13
        {
            //等待抬键
            //while ((act_readl(SYSTEM_CTL_RTCVDD) & 0x80000000) != 0) //gl5110 hjh 2015-10-13
            while ((act_readl(ONOFF_KEY) & 0x1) != 0) //gl5116 hjh 2015-10-13
            {
                ;//do nothing
            }
    ...
    //act_writel(bak_clk, CLKENCTL); //gl5110 hjh 2015-10-17
    act_writel(bak_clk, CMU_DEVCLKEN); //gl5116 hjh 2015-10-17
...
void _display_charging(void)
{
    ...
    uint8 real_battery_value; //当前电池电量
    battery_status_e tmp_charge_state; 
    if((tmp_charge_state =  key_chargeget(&real_battery_value)) == BAT_FULL)
    {
        //充满电，亮背光30s
...
/*            if (private_msg.msg.type == MSG_RTCALARM)
        ...
        sys_os_time_dly(2);//延时20ms,释放时间片
    }
