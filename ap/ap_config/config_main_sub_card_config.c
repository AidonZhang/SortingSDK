...
void card_set_drv_power(void)
{
    ...
    /*增加SD 卡单线\四线、CLK0\CLK1选择配置*/
	card_pm_param.reserved0 = (uint8)com_get_config_default(SETTING_CARD_LINE_WIDTH);
   	card_pm_param.reserved0 += ((uint8)com_get_config_default(SETTING_CARD_CLK_CHOICE) << 1);
