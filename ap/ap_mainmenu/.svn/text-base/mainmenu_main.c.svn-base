...
static void _read_var(void)
{
    ...
    /*第一次进入，初始化VM变量*/
    if (g_mainmenu_var.magic != VRAM_MAGIC(VM_AP_MAINMENU))
    {
        g_mainmenu_var.magic = VRAM_MAGIC(VM_AP_MAINMENU);
        ...
        sys_vm_write(&g_mainmenu_var, VM_AP_MAINMENU, sizeof(g_mainmenu_var_t));
    }
    /*读出各应用文件总数*/
...
static void _app_exit(void)
{
    ...
    /*保存VM变量*/
    sys_vm_write(&g_mainmenu_var, VM_AP_MAINMENU, sizeof(g_mainmenu_var));
    sys_vm_write(&g_setting_comval, VM_AP_SETTING, sizeof(g_setting_comval));
