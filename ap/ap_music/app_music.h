...
//系统默认隐藏列表文件*.LIB,*.PIC,*.PL,debug时先关闭
...
extern uint8 bm_buf[SECTOR_SIZE] _BANK_DATA_ATTR_;
...
typedef struct
{
    ...
    //uint8 direct;
    /*! 是否需要滚屏 置1需要,0不需要*/
    uint8 need_scroll; // bit0 -> title, bit1 -> artist, bit2 ->album;
} scroll_vars_t;
...
//歌词存在标识
extern uint8 g_lyric_flag;

//歌词编码类型
extern uint8 g_lyric_charcode;
...
//歌曲格式，比特率和采样率等信息缓存
extern uint8 g_music_info_buf[22];

//滚屏内容
extern scroll_content_t g_scroll_content[4];
...
//播放状态处理标志
extern uint32 g_status_deal_flag;
...
//支持APE cue列表播放开关
//#define SUPPORT_ApeCue_SWITCH

//需要ape cue列表播放
#ifdef SUPPORT_ApeCue_SWITCH

    //cue文件信息结构体
    extern cue_info_t g_cue_file;

    //浏览播放起始时间
    extern uint32 g_cue_intro_start;
        
    //cue随机播放曲目 最大支持32首cue歌曲
    extern uint32 g_cue_shuffle_num;
...
//for ape cue 
extern app_result_e handle_cue(uint8 active)
__FAR__;
extern app_result_e ui_show_cue(msg_cb_func msg_call_back)
__FAR__;
extern void music_load_cuefile(void)
__FAR__;
extern app_result_e set_playmode_apecue(void)
__FAR__;
extern void set_scene_cuemode(uint8 mode)
__FAR__;
...
extern app_result_e music_scene_setmenu(music_setmenu_e setmenu_type);
...
extern bool music_set_eq(eq_param_t *eq_val);
...
extern bool music_play_cuemusic(mmm_mp_bp_info_t *bp_ape)
__FAR__;
...
//extern bool music_get_audible_image(mengine_playinfo_t* play_info)
//__FAR__;
...
extern void check_albumart_file_exist(void);

#ifndef  _CARD_BASE_
extern int ext_vm_read(uint8 *pbuf, unsigned int vm_offset, unsigned int len)__FAR__;
extern int ext_vm_write(uint8 *pbuf, unsigned int vm_offset)__FAR__;
#endif
