...
{
    { LISTMENU_ENTRY, S_ALLSONG, 0 }, //list menu入口菜单 1

    { LISTMENU_ENTRY_DIR, S_LOCAL_FOLDER, 0 }, //list menu入口菜单(卡存在的情况下) 2

    { OPTION_MENU_MPLAYING, S_NOW_PLAYING, S_MUSIC },//list menu option music正在播放 (audible列表 相同)3
    { OPTION_MENU_MPAUSE, S_LAST_PLAY, S_MUSIC },//list menu option music上一次播放(audible列表 相同)4
    { OPTION_MENU_RPLAYING, S_NOW_PLAYING, S_RADIO },//list menu option radio正在播放(audible列表 相同)5
    { OPTION_MENU_RPAUSE, S_LAST_PLAY, S_RADIO },//list menu option radio上一次播放(audible列表 相同)6
    ...
    { MUSICSET_MENU, S_SET_MENU, 0 }, //music set menu入口菜单43

    { AUDIBLESET_MENU, S_BOOKMARK, 0 }, //audible set menu入口菜单
    
    { APESET_MENU, S_SET_APE, 0 }, //ape set menu入口菜单45
    
    { SLEEP_TIMER_SETMENU, S_SLEEP_TIMER, 0 }, //已设置定时器入口菜单46

    { SHUFFLE_PLAY, S_SHUFFLE_ALL, 0 }, //title列表中菜单项入口菜单47

    { ALLSONG_PLAY, S_ALLSONG, S_LIST_OPTION1 }, //artist & genre列表中菜单项入口菜单48

    { VOICE_MENU_PLAYING, S_NOW_PLAYING, 0},//49

    { VOICE_MENU_LASTPLAY, S_LAST_PLAY, 0},//50
    
    { M3ULIST_MENU_FAVOR, S_NOW_FAVORITE, 0}, //m3u list嵌套favorlist入口菜单51
    ...
    { LISTMENU_ENTRY_NO_GENRE, S_ALLSONG_NO_GENRE, 0 }, //list menu入口菜单 1
...
const conf_menu_item_t leafitems[] =
{
    ...
    //audible set menu
    { S_BOOKMARK, 0, NULL, NULL, set_scene_leveltop_option, NORMAL_MENU_ITEM , 65, 0}, //书签菜单(是根菜单)
    //添加书签菜单
    ...
    { S_SET_APE, 0, NULL, NULL, set_scene_leveltop_option, NORMAL_MENU_ITEM , 73, 0}, //书签菜单(是根菜单)
