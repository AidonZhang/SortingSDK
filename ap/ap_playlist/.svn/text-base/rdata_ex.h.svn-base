...
extern uint32 cmu_memclken_bak;
extern uint32 cmu_memclksel_bak;
//*lib file's info
...
//文件写指针
extern uint8 *title_file_pt;
extern uint8 *album_file_pt;
extern uint8 *artist_file_pt;
extern uint8 *genre_file_pt;
extern uint8 *track_file_pt;

//ram 写指针
extern uint8 *title_data_pt;
extern uint8 *fileinfo_data_pt;
extern uint8 *track_data_pt;
extern uint16 *resort_data;
...
extern uint8 track_string_buf[ID3_TRACK_SIZE];//用来缓存track string
...
extern uint8 sort_cmp_len;  //第一遍能比较的最大长度
...
extern uint16 ap_plist_sorting_level;
