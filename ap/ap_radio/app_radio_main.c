...
void radio_load_cfg(void)
{
    ...
    /*初始化系统的VM变量*/
    if (temp_comval.g_comval.magic != VRAM_MAGIC(VM_AP_SETTING))
    {
        temp_comval.g_comval.magic = VRAM_MAGIC(VM_AP_SETTING);
        com_get_default_setting(&temp_comval);
        sys_vm_write(&temp_comval, VM_AP_SETTING, sizeof(setting_comval_t));
    }
...
void radio_save_cfg(void)
{
    ...
    /*保存VM变量*/
    sys_vm_write(&temp_comval, VM_AP_SETTING, sizeof(setting_comval_t));
    sys_vm_write(&g_radio_config, VM_AP_RADIO, sizeof(radio_config_t));

    /*读出各应用文件总数*/
    sys_vm_read(&ap_file_num, VM_FILE_NUM, sizeof(ap_file_num));
    ap_file_num[APP_ID_RADIO] = g_radio_config.FMStatus.station_count;
    sys_vm_write(&ap_file_num, VM_FILE_NUM, sizeof(ap_file_num));
...
void radioUI_scene_result_deal(app_result_e result_val)
{
    ...
    switch (result_val)
    {
        ...
        need_create_app = FALSE;
        //applib_start_playlist_builder(APP_ID_PLAYLIST, PARAM_FROM_RADIO|PLIST_DISK_H);
        msg.content.data[0] = APP_ID_PLAYLIST;
        msg.content.data[1] = PARAM_FROM_RADIO | PLIST_DISK_H;
        break;
        //默认回到main
        default:
        msg.content.data[0] = APP_ID_MAINMENU;//main ap
