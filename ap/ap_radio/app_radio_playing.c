...
void ui_show_freqbar(uint16 freq, uint8 mode)
{
    ...
    if (g_radio_config.band_mode == Band_Japan)
    {
        ...
        freq_progress.total =240; //140;//140;
    }
    //其他波段范围87.5M~~108M
    else
    {
    ...
    while (1)
    {
        ...
        if (get_screensave_state() == FALSE)//不在屏保模式才会显示
        {
            ...
            if ((paint_flag & PAINT_ALL) == PAINT_ALL)
            {
                ...
                true_freq = (uint32)((1 << 16) + g_radio_config.FMStatus.freq);
#if 0
                //背景
                ...
                //FM字段
                ui_show_pic(FM_FM, 190, 50);
                //fm_arrow
                style_infor.style_id = FM_LARROW1;
                style_infor.type = UI_AP;
                arrow_pic.pic_id = V_U16_INVALID;
                arrow_pic.frame_id = 0;
                ui_show_picbox(&style_infor, &arrow_pic);
                style_infor.style_id = FM_RARROW1;
                style_infor.type = UI_AP;
                arrow_pic.pic_id = V_U16_INVALID;
                arrow_pic.frame_id = 0;
                ui_show_picbox(&style_infor, &arrow_pic);
...
//            if ((first_wait == TRUE) && (radio_get_antenna() == TRUE))
            if ((first_wait == TRUE) && (radio_detect_antenna() == TRUE))                
            {
