 ...
 * \void 
 * \判断选择哪个入口菜单
 * \param[in]    record_bitrate_e 比特率值
 * \param[out]   void
 * \return       void
 * \retval       void 
 ...
 */
/*******************************************************************************/
 static void set_bitrate(record_bitrate_e value)
 {
    switch(g_record_vars.file_type)
    {
        case REC_FORMAT_WAV:
        g_record_vars.wav_bitrate = value;
        break;
        case REC_FORMAT_MP3:
        g_record_vars.mp3_bitrate = value;
        break;
        case REC_FORMAT_PCM_WAV:
        g_record_vars.pcm_wav_bitrate = value;
        break;
        case REC_FORMAT_ADPCM_WAV:
        g_record_vars.adpcm_wav_bitrate = value;
        break;
    }
    g_record_vars.bitrate = value;    
}
/******************************************************************************/
/*!
 * \par  Description:
...
app_result_e set_rec_format(void *title_action)
{
    ...
    else if(REC_FORMAT_PCM_WAV == g_record_vars.file_type)
    {
        menu_title_rec_format->str_id = SET_REC_FORMAT_WAV_PCM;
    }else if(REC_FORMAT_ADPCM_WAV == g_record_vars.file_type)
    {
    	menu_title_rec_format->str_id = SET_REC_FORMAT_WAV_ADPCM;
    }
...
app_result_e set_rec_format_pcmwav(void* param)
{
    g_record_vars.file_type = REC_FORMAT_PCM_WAV;
    g_record_vars.bitrate = g_record_vars.pcm_wav_bitrate;
    return RESULT_MENU_PARENT;
}

app_result_e set_rec_format_adpcmwav(void* param)
{
    g_record_vars.file_type = REC_FORMAT_ADPCM_WAV;
    g_record_vars.bitrate = g_record_vars.adpcm_wav_bitrate;
    return RESULT_MENU_PARENT;
}
...
app_result_e set_rec_source(void *title_action)
{
    ...
    if (AI_SOURCE_ASEMIC == g_record_vars.ai_source ||AI_SOURCE_ADFMIC ==  g_record_vars.ai_source)
    {
...
/*******************************************************************************/
 app_result_e set_rec_source_mic(void* param)
 {
    g_record_vars.ai_source = AI_SOURCE_ASEMIC;
    return RESULT_MENU_PARENT;
}
 ...
 */
 app_result_e get_rec_bitrate_entry(void *title_action)
 {
    ...
    {
    	menu_title_rec_bitrate->menu_entry = ENTRY_BITRATE_PCM_WAV;
        switch (g_record_vars.pcm_wav_bitrate)
        {
            case BIT_512KBPS:
            menu_title_rec_bitrate->str_id = SET_REC_BITRATE_512KBPS;
            break;
    ...
    {
       menu_title_rec_bitrate->menu_entry = ENTRY_BITRATE_ADPCM_WAV;
       switch (g_record_vars.adpcm_wav_bitrate)
       {
        case BIT_32KBPS:
        menu_title_rec_bitrate->str_id = SET_REC_BITRATE_32KBPS;
        break;
