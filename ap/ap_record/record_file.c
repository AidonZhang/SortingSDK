...
//录音后缀名
static const uint8 rec_ext[][6] =
{ "WAV", "MP3", "ACT","WAV","WAV" }; //wav格式和MP3格式 + PCM + ADPCM都是wav后缀
...
static int8 _record_parse_filenum(void)
{
    ...
    {
        g_file_total = fsel_get_total(TRUE);
        g_first_enter_flag = FALSE;
    }

    /*if (g_file_total >= FILE_NUM_MAX) //2015-11-12
    {
        return -1;
    }
...
static bool _record_generate_filename(void)
{
   ...
   // loops = 0; //2015-11-12
    ...
    if (PARAM_FROM_MAINMENU == g_entry_mode)
    {
        ...
        /*if (num > FILE_NUM_MAX) //2015-11-12
        {
            num = 1;
        }*/
   ...
   // } while (loops < FILE_NUM_MAX); //2015-11-12
