...
static void _read_record_var(void)
{
    ...
    /*初始化系统的VM变量*/
    if (temp_val.g_comval.magic != VRAM_MAGIC(VM_AP_SETTING))
    {
    ...
    if (g_record_vars.maigc != VRAM_MAGIC(VM_AP_RECORD))
    {
        //设定默认值
        g_record_vars.maigc = VRAM_MAGIC(VM_AP_RECORD);
        ...
        g_record_vars.pcm_wav_bitrate = (uint8) com_get_config_default(RECORD_PCM_WAV_BITRATE);//BIT_32KBPS;
        g_record_vars.adpcm_wav_bitrate = (uint8) com_get_config_default(RECORD_ADPCM_WAV_BITRATE);//BIT_32KBPS;
        ...
        g_record_vars.listening_flag = (uint8) com_get_config_default(RECORD_LISTENING_ENABLE);//3
        g_record_vars.listening_volume = (uint8) com_get_config_default(RECORD_LISTENING_VOLUME);//3
    }
    //读出alarm信息
    _read_fmrecord_var();
    //磁盘初始化
...
static void _save_record_var(void)
{
    ...
    sys_vm_write(&g_record_vars, VM_AP_RECORD, sizeof(g_record_vars));
...
app_result_e _scene_dispatch(app_param_e enter_mode)
{
    ...
    g_entry_mode = PARAM_FROM_MAINMENU;
    switch (g_entry_mode)
    {
        case PARAM_FROM_MAINMENU:
        g_scene_param.record_scene = SCENE_MENU_REC;
      //  g_scene_param.record_scene = SCENE_UI_REC;
 //       g_record_vars.ai_source = AI_SOURCE_ADFMIC;
//       libc_print("ai source",g_record_vars.ai_source,2);
 ...
 */
/*******************************************************************************/
__section__(".text._scene_result_deal")void _scene_result_deal(app_result_e result_val)
{
    msg_apps_t msg;
    switch (result_val)
    {
...
/*        if (get_engine_type() == ENGINE_RADIO)
        ...
        msg.content.data[1] = PARAM_FROM_RECORD|PLIST_DISK_H;
