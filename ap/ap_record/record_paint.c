...
void _record_scene_paint(uint32* pdisplay_bitmap)
{
    ...
    /*显示语音录音字符*/
    if ((*pdisplay_bitmap & up_recordTxt) != 0)
    {
        style.style_id = STY_RECORDING;
        style.type = UI_AP;

        textbox_data.lang_id = UNICODEID;
        textbox_data.str_value = NULL;
        textbox_data.str_id = RECORDING;

        ui_show_textbox((void*) &style, (void*) &textbox_data, TEXTBOX_DRAW_NORMAL);

        /*清显示标志*/
        *pdisplay_bitmap &= clr_recordTxt;
    }
