...
uint8 SN_buffer[SN_SIZE];   //存放SN数据
...
void read_var(void)
{
    ...
    /*第一次进入，初始化VM变量*/
    if (g_setting_vars.g_comval.magic != VRAM_MAGIC(VM_AP_SETTING))
    {
        libc_memset(&g_setting_vars,0,sizeof(setting_comval_t));
        g_setting_vars.g_comval.magic = VRAM_MAGIC(VM_AP_SETTING);
        ...
        sys_vm_write(&g_setting_vars, VM_AP_SETTING, sizeof(g_setting_vars));
    }
...
bool setting_app_init(void)
{
    ...
    //初始化文件选择器
    #ifdef _CARD_BASE_
    g_file_path_info.file_path.plist_location.disk = DISK_C; //DISK_H 卡盘 DISK_U uhost 盘//2015-11-25
    current_disk = DISK_C; 
    #else
    g_file_path_info.file_path.plist_location.disk = DISK_H;//DISK_C; //DISK_H 卡盘 DISK_U uhost 盘//2015-11-25
    current_disk = DISK_H; //DISK_C; //2015-11-25
    #endif
    //初始化文件选择器为目录模式，一定成功
...
static app_result_e setting_proc_result(app_result_e result)
{
    ...
    switch (result)
    {
        ...
        //temp_flag = 1;
        //applib_start_playlist_builder(APP_ID_PLAYLIST, PARAM_FROM_SETTING|PLIST_DISK_H);
        msg.content.data[0] = APP_ID_PLAYLIST;
        #ifdef _CARD_BASE_
        msg.content.data[1] = PARAM_FROM_SETTING|PLIST_DISK_C;
        #else
        msg.content.data[1] = PARAM_FROM_SETTING|PLIST_DISK_H;
        #endif
        break;
        case RESULT_AUTO_UPGRADE:
        //libc_print("result_auto_upgrade",0,0);
