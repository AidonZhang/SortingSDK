...
app_result_e show_disk_information_callback(void *menu_title_void)
{
    ...
    count = libc_itoa((uint32) usabel_space_sec, usable_space, 0); //将可用空间转成字符串
   
    
    count = libc_itoa((uint32) total_space_sec, total_space, 0); //将总空间转成字符串
    
    
    count = libc_itoa((uint32) g_ap_file_num[APP_ID_MUSIC], total_musics, 0); //将音乐总数转成字符串
    ...
    string_name[2].data.str = total_musics; //歌曲总数最大4000首
