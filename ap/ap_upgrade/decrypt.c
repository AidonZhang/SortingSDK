...
uint32 _read_file_translate(uint8 *read_buffer, uint32 read_len)
{
    ...
    //libc_print("roff9",read_file_offset,2); 
    ...
    while (need_read_len != 0)
    {
        ...
        //if (true_read == 1)
        {
            ...
            if (decrypt_mode == 0)
            {
                ...
                PRINT_INFO("firt decrypt over1");
            }
            else
            {
                //((p_fw_decrypt_run) g_decrypt_ops.fw_decrypt_run)(g_decrypt.pInOutBuffer, (void *) once_read_len,
                //        g_decrypt.pGLBuffer);
                PCFWDecrypt(g_decrypt.pInOutBuffer, (long)once_read_len, g_decrypt.pGLBuffer);
            }
        }
...
void decrypt_init(void)
{
    ...
    libc_print("len",g_decrypt.FileLength,2); //2015-12-03
}
...
uint32 _read_file(uint32 read_len)
{
    uint32 read_bytes;
    upgrade_get_app_msg();
    read_bytes = _read_file_translate((uint8 *)BUFFER_ADDR, read_len);
    return read_bytes;
}
