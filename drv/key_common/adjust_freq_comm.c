...
const uint8 b_level_table_sn[2][FREQ_LEVEL_MAX] =
{
    ...
    { 00, 35, 39, 55, 60, 70, 80, 100, 120, 140, 160, 180 }, //1
};
...
int adjust_freq_update(void)
{
    ...
    max_freq = (freq_a >  freq_b)? freq_a: freq_b;

    if(max_freq < 60)
    {
        lcd_clk_div = 0x00000000;
    }
    else if ( max_freq < 100)
    {
        lcd_clk_div = 0x00000101;       
    }
    else
    {
        lcd_clk_div = 0x00000202;       
    }
    
    act_writel(lcd_clk_div, LCD_CLKCTL);
    act_writel(lcd_clk_div, EXTMEM_CLKCTL);         
            
    /*    if(freq_b >= 80 || freq_b >= 80)
    {
            act_writel(0x00000101, LCD_CLKCTL);
            act_writel(0x00000101, EXTMEM_CLKCTL);

    }
    else{
        act_writel(0x00000000, LCD_CLKCTL);
        act_writel(0x00000000, EXTMEM_CLKCTL);  
    }
    */
    return 0;
}
...
int adjust_freq_do_task_level(int prio, int level)
{
    ...
    if ((AP_BACK_HIGH_PRIO == prio) || (AP_FRONT_LOW_PRIO == prio))
    {
        //关联DSP频率
