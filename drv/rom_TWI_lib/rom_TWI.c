...
void TWI_Init(gpio_init_cfg_t* gpio, uint8* delay)
{
   ...
   // TWI_SCL_Input_En(gpio); GPIO 不能同时输入 输出
...
uint8 TWI_GetAck(gpio_init_cfg_t* gpio, uint8* delay)
{
    ...
    else
    {
 	...
 	TWI_Delay(delay);
	TWI_SCL_low_hold(gpio, delay); // before TWI_SDA_Output_En(gpio) 2015-12-28 by shenlei ;
	TWI_SDA_Output_En(gpio);
	TWI_SDA_low_hold(gpio, delay);
	TWI_SDA_Input_Dis(gpio);
   ...
   // TWI_SCL_low_hold(gpio, delay);  2015-12-28 by shenlei 
   // TWI_SDA_low_hold(gpio, delay);
    //TWI_SDA_Input_Dis(gpio);
