        ...
        res_open_infor_t *style_ptr)
{
    ...
    if (type == 1)
    {
        if ((list_item.attrib & ITEMS_ACTIVE_BG_OPTION) == ACTIVE_BG_OPTION_SPIC)
        {
            ...
            if (list_item.back_act_id != (uint16) - 1)
            {
                ...
                romf_show_pic(list_item.back_act_id, list_item.back_x, back_y);
            }
        }
        else if ((list_item.attrib & ITEMS_ACTIVE_BG_OPTION) == ACTIVE_BG_OPTION_MPIC)
        {
            ...
            if (list_item.back_act_id != (uint16) - 1)
            {
                ...
                romf_show_pic(list_item.back_act_id + tmp, list_item.back_x, back_y);
            }
        }
        else
        {
         ...
         && ((cur_item->argv & ITEMS_FRONT_TAG_ARGV) == ITEMS_FRONT_TAG_IS_DISP))
        {
            ...
            {
                romf_show_pic(list_item.head_act_id, list_item.head_x, head_y);
            }
            else
            {
                if ((cur_item->argv & ITEMS_FRONT_TAG_INDEX) != ITEMS_FRONT_TAG_INDEX)
                {
                    romf_show_pic(listbox_data->head_act_id + \
                    ((cur_item->argv & ITEMS_FRONT_TAG_INDEX)>>ITEMS_FRONT_TAG_INDEX_SHIFT),list_item.head_x, head_y);
                }
                else
                {
                    romf_show_pic(list_item.head_act_id + itemnum, list_item.head_x, head_y);
                }
            }
        }
        ...
        if ((cur_item->argv & ITEMS_BACK_TAG_ARGV) == ITEMS_BACK_TAG_IS_SELECT)
        {
            ...
            {
                romf_show_pic(list_item.tail_act_id, list_item.tail_x, tail_y);
            }
            else if ((list_item.attrib & ITEMS_BACK_TAG_OPTION) == BACK_TAG_OPTION_MULTY)
            {
                romf_show_pic(list_item.tail_act_id + itemnum, list_item.tail_x, tail_y);
            }
            else
            {
                //do nothing for QAC
    ...
    else
    {
        if ((list_item.attrib & ITEMS_INACTIVE_BG_OPTION) == INACTIVE_BG_OPTION_SPIC)
        {
            ...
            {
                romf_show_pic(list_item.back_id, list_item.back_x, back_y);
            }
        }
        else if ((list_item.attrib & ITEMS_INACTIVE_BG_OPTION) == INACTIVE_BG_OPTION_MPIC)
        {
            ...
            if (list_item.back_id != (uint16) - 1)
            {
                romf_show_pic(list_item.back_id + tmp, list_item.back_x, back_y);
            }
        }
        else
        {
             ...
             && ((cur_item->argv & ITEMS_FRONT_TAG_ARGV) == ITEMS_FRONT_TAG_IS_DISP))
        {
            ...
            {
                romf_show_pic(list_item.head_id, list_item.head_x, head_y);
            }
            else
            {
                if ((cur_item->argv & ITEMS_FRONT_TAG_INDEX) != ITEMS_FRONT_TAG_INDEX)
                {
                    romf_show_pic(listbox_data->head_id + \
                    ((cur_item->argv & ITEMS_FRONT_TAG_INDEX)>>ITEMS_FRONT_TAG_INDEX_SHIFT),list_item.head_x, head_y);
                }
                else
                {
                    romf_show_pic(list_item.head_id + itemnum, list_item.head_x, head_y);
                }
            }
        }
        ...
        if ((cur_item->argv & ITEMS_BACK_TAG_ARGV) == ITEMS_BACK_TAG_IS_SELECT)
        {
            ...
            {
                romf_show_pic(list_item.tail_id, list_item.tail_x, tail_y);
            }
            else if ((list_item.attrib & ITEMS_BACK_TAG_INA_OPTION) == BACK_TAG_INA_OPTION_MULTY)
            {
                romf_show_pic(list_item.tail_id + itemnum, list_item.tail_x, tail_y);
            }
            else
            {
                //do nothing for QAC
