...
ui_result_e show_dialogbox(style_infor_t *dialogbox_style, dialog_private_t *dialogbox_data, dialog_draw_mode_e mode)
{
    ...
    if (mode == DIALOG_DRAW_ALL)
    {
        ...
        {
            romf_show_pic(dialogbox.back_id, dialogbox.back_x, dialogbox.back_y);
        }
        //需要显示items
        if ((dialogbox.attrib & DIALOG_SHOW_ICON_EN) == DIALOG_SHOW_ICON_EN)
        {
            ...
            {
                romf_show_pic(dialogbox_data->icon_id, dialogbox.icon_x, dialogbox.icon_y);
            }
            else if (dialogbox.icon_id != (uint16) - 1)
            {
                romf_show_pic(dialogbox.icon_id, dialogbox.icon_x, dialogbox.icon_y);
            }
            else
            {
                //do nothing for QAC
        ...
        res_open_infor_t *style_ptr)
{
    ...
    if (type == 0)
    {
        ...
        if ((dialogbox_item.attrib & ITEMS_INACTIVE_BG_OPTION) == INACTIVE_BG_OPTION_SPIC)
        {
            ...
            romf_show_pic(dialogbox_item.backgd_id, dialogbox_item.item_x[itemnum], dialogbox_item.item_y[itemnum]);
        }
        //图片方式（multy）
        else if ((dialogbox_item.attrib & ITEMS_INACTIVE_BG_OPTION) == INACTIVE_BG_OPTION_MPIC)
        {
            disp_mode = CHAR_FILL_NORMAL; //有背景图片，则是透明模式
            romf_show_pic(dialogbox_item.backgd_id + itemnum, dialogbox_item.item_x[itemnum],
    ...
    else
    {
        ...
        {
            disp_mode = CHAR_FILL_NORMAL; //有背景图片，则是透明模式
            romf_show_pic(dialogbox_item.act_backgd_id, dialogbox_item.item_x[itemnum], dialogbox_item.item_y[itemnum]);
        }
        //图片方式（multy）
        else if ((dialogbox_item.attrib & ITEMS_ACTIVE_BG_OPTION) == ACTIVE_BG_OPTION_MPIC)
        {
            disp_mode = CHAR_FILL_NORMAL; //有背景图片，则是透明模式
            romf_show_pic(dialogbox_item.act_backgd_id + itemnum, dialogbox_item.item_x[itemnum],
                    dialogbox_item.item_y[itemnum]);
        }
        //背景颜色，填充为按钮效果
        else
