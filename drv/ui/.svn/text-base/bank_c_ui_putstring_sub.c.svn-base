...
#if (USE_NEW_FONTLIB && FONT_ANTIALIAS)
{
    region_t  charRect;

    charRect.x      = start_col;
    charRect.y      = start_row;
    charRect.width  = region.width;
    charRect.height = end_row - start_row;
    
    Font_DrawChar(display_buffer_base, buf_region, psoure_buf, &charRect, char_disp, mode);
}
#else
...
{
#if (USE_NEW_FONTLIB)
    Font_GetCharBits(FontLib_fp, code_rename, picbuf);
#else
