...
ui_result_e show_parambox(style_infor_t *parambox_style, parambox_private_t *parambox_data, parambox_draw_mode_e mode)
{
    ...
    if (mode == PARAMBOX_DRAW_ALL)
    {
        ...
        {
            romf_show_pic(parambox_data->back_id, parambox.back_x, parambox.back_y);
        }
        else if (parambox.back_id != (uint16) - 1)
        {
            romf_show_pic(parambox.back_id, parambox.back_x, parambox.back_y);
        }
        else
        {
            //do nothing for QAC
        ...
        for(i = 0; i < 2; i++)
        {
            ...
            {
                romf_show_pic(parambox.param_part_id[i], parambox.param_part_x[i], parambox.param_part_y[i]);
            }
        }
        ...
        if ((parambox.attrib & PARAMBOX_SHOW_TAGPIC_EN) == PARAMBOX_SHOW_TAGPIC_EN)
        {
            ...
            {
                romf_show_pic(parambox_data->icon_id, parambox.icon_x, parambox.icon_y);
            }
            else if (parambox.icon_id != (uint16) - 1)
            {
                romf_show_pic(parambox.icon_id, parambox.icon_x, parambox.icon_y);
            }
            else
            {
                //do nothing for QAC
