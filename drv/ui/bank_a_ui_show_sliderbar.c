...
ui_result_e show_sliderbar(style_infor_t *parambox_style, parambox_private_t *parambox_data, parambox_draw_mode_e mode)
{
    ...
    if (mode == PARAMBOX_DRAW_ALL)
    {
        ...
        {
            romf_show_pic(parambox_data->back_id, sliderbar.back_x, sliderbar.back_y);
        }
        else if (sliderbar.back_id != (uint16) - 1)
        {
            romf_show_pic(sliderbar.back_id, sliderbar.back_x, sliderbar.back_y);
        }
        else
        {
            //do nothing for QAC
        ...
        if ((sliderbar.attrib & SLIDERBAR_SHOW_TAGPIC_EN) == SLIDERBAR_SHOW_TAGPIC_EN)
        {
            ...
            {
                romf_show_pic(parambox_data->icon_id, sliderbar.icon_x, sliderbar.icon_y);
            }
            else if (sliderbar.icon_id != (uint16) - 1)
            {
                romf_show_pic(sliderbar.icon_id, sliderbar.icon_x, sliderbar.icon_y);
            }
            else
            {
                //do nothing for QAC
        ...
        {
            romf_show_pic(sliderbar.addition_pic, sliderbar.addition_x, sliderbar.addition_y);
        }
