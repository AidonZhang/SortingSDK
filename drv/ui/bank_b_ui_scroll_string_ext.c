...
bool scroll_string_ext(scroll_content_t *scroll_string_infor, scroll_param_t *scroll_param, void *null3)
{
    ...
    if (scroll_param->init == TRUE)
    {
        ...
        textbox_data.lang_id = scroll_string_infor->language;
        textbox_data.str_value = scroll_string_infor->strbuf;
    ...
    else
    {
        if(show_textbox(NULL, NULL, TEXTBOX_DRAW_SCROLL) == UI_SCROLL_OVER)
        {
            ...
            textbox_data.lang_id = scroll_string_infor->language;
            textbox_data.str_value = scroll_string_infor->strbuf;
            show_textbox(scroll_param->id3_text, &textbox_data, TEXTBOX_DRAW_NORMAL);
        }
        else
        {
            return TRUE;
