        ...
        progress_draw_mode_e mode)
{
    ...
    uint16 tmp_prev_num;
    uint16 tmp_y;
    res_open_infor_t *progressbar_bg_res_ptr;
    static res_entry_t progressbar_bg_res_entry;
    ...
    {
         //刷背景并获得processbar 高度
        if (progressbar.back_id != (uint16) - 1) //2015-11-16
        {
            progressbar_bg_res_ptr = res_ptr_select(progressbar.back_id);
            sys_sd_fseek(progressbar_bg_res_ptr->res_fp, SEEK_SET, progressbar_bg_res_ptr->pic_entry_offset 
                + (uint32) (REAL_RES_ID(progressbar.back_id) * RESHEADITEM));
            sys_sd_fread(progressbar_bg_res_ptr->res_fp, &progressbar_bg_res_entry, sizeof(res_entry_t));
            romf_show_pic(progressbar.back_id, progressbar.back_x, progressbar.back_y);
        }
          ...
          || ((progressbar.attrib & PROGRESS_STYLE_OPTION) == STYLE_OPTION_BOTH))
    {
        ...
        if((progressbar.attrib & PROGRESS_DIRECTION_OPTION) == PROGRESS_DIRECTION_HORIZONTAL)
        {
            ...
            if (width_offset > tmp_prev_num)
            {
                ...
                fill_region.height = progressbar_bg_res_entry.height;//6;//进度条高度
                progressbar.prog_start_x = fill_region.x + fill_region.width;

                romf_fill_rect(&fill_region, 0x7e48); //绿色
            ...
            else if (width_offset < tmp_prev_num)
            {
                ...
                romf_fill_rect(&fill_region, 0x8430); //浅灰色
