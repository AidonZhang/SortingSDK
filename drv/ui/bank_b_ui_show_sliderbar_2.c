        ...
        parambox_draw_mode_e mode, res_open_infor_t *style_ptr)
{
    ...
    if ((mode == PARAMBOX_DRAW_ALL) || (mode == PARAMBOX_DRAW_PARAM))
    {
        ...
        romf_show_pic(slider_back_id, back_x, sliderbar_slider.slider_back_y);
    }
            ...
            & SLIDER_STYLE_OPTION) == STYLE_OPTION_BOTH))
    {
        if ((sliderbar_slider.attrib & SLIDER_DIRECTION_OPTION) == DIRECTION_OPTION_HORIZONTAL) //横向刷新
        {
            romf_show_pic(sliderbar_slider.prog_id, prog_x + slider_cur_offset, sliderbar_slider.slider_start_y);
            if (mode == PARAMBOX_DRAW_VALUE)
            {
                romf_show_pic(sliderbar_slider.unprog_id, prog_x + tmp_slider_prev_offset, \
                    sliderbar_slider.slider_start_y);
            }
        }
        else //竖向
        {
            romf_show_pic(sliderbar_slider.prog_id, prog_x, sliderbar_slider.slider_start_y - slider_cur_offset);
            if (mode == PARAMBOX_DRAW_VALUE)
            {
                romf_show_pic(sliderbar_slider.unprog_id, prog_x, \
    ...
    && (prog_flag == 0)) //进度减少时，重画刻度线
    {
        ...
        while (i > 0)
        {
            if ((sliderbar_slider.attrib & SLIDER_DIRECTION_OPTION) == DIRECTION_OPTION_HORIZONTAL) //横向刷新
            {
                romf_show_pic(sliderbar_slider.slider_fill_id, fill_x, sliderbar_slider.slider_start_y);
            }
            else
            {
                if (prog_direct_flag == 1)
                {
                    romf_show_pic(sliderbar_slider.slider_fill_id, fill_x, fill_y + scale);
                }
                else
                {
                    romf_show_pic(sliderbar_slider.slider_fill_id, fill_x, fill_y);
                }
            }
            if (tmp_slider_prev_offset < 0)
            {
